...
- **HDM模型**——基于"分层确定性标准模型"建立的账户体系，用户只需一个“恢复密语和密码”创建的钱包，就可最多建立约21亿的子账户地址。
...
- **iCloud备份**——钱包的“账户体系数据”可开启iCloud自动同步，使用“恢复密语和密码”进行恢复会自动同步对应的“账户体系数据”，即可找回所丢失的HDM账户和多重签名账户。既保证私钥的安全，也为用户提供了便捷的数据迁移到新设备的方式。
