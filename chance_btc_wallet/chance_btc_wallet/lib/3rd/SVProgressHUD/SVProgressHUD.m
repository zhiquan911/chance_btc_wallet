...
+ (void)setAnimatedViewColor:(UIColor*)color {
    [self sharedView].animatedViewColor = color;
}
...
+ (void)showInfoWithStatus:(NSString*)status duration:(NSTimeInterval)duration {
    [[self sharedView] showImage:[self sharedView].infoImage status:status duration:duration];
}
...
+ (void)showSuccessWithStatus:(NSString*)status duration:(NSTimeInterval)duration {
    [[self sharedView] showImage:[self sharedView].successImage status:status duration:duration];
}
...
- (UIView*)indefiniteAnimatedView {
    ...
    if(self.defaultAnimationType == SVProgressHUDAnimationTypeFlat){
        ...
        indefiniteAnimatedView.strokeColor = self.animatedViewColor;
...
- (SVProgressAnimatedView*)ringView {
    ...
    // Update styling
    _ringView.strokeColor = self.animatedViewColor;
...
+ (NSTimeInterval)displayDurationForString:(NSString*)string {
    return MAX((float)string.length * 0.16 + 0.5, [self sharedView].minimumDismissTimeInterval);
}
...
- (UIColor*)animatedViewColorForStyle {
    if(self.defaultStyle == SVProgressHUDStyleLight) {
        return [UIColor blackColor];
    } else if(self.defaultStyle == SVProgressHUDStyleDark) {
        return [UIColor whiteColor];
    } else {
        return self.animatedViewColor;
    }
}
...
- (void)setAnimatedViewColor:(UIColor*)color {
    if (!_isInitializing) _animatedViewColor = color;
}
